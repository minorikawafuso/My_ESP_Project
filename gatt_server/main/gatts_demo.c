...
static void do_retransmit(const int sock)
{
    ...
    do {
        ...
        if (len < 0) {
            ...
            //for(int i = 32; i < 96; i++) sta_config.sta.password[i] = rx_buffer[i];
            //printf("pwd: %s\n", pwd);
            //sta_config.sta.threshold.authmode=ESP_WIFI_
            wifi_config_t sta_config = {
                .sta = {
                    // .ssid = ssid,
                    // for(int i = 0; i < 32; i++).ssid[i] = ssid[i]
                    .password = "",
                //.threshold.authmode = ESP_WIFI_SCAN_AUTH_MODE_THRESHOLD,
                },
            };
            for(int i = 0; i < 32; i++) sta_config.sta.ssid[i] = rx_buffer[i];
